...
public class KrakenApi {
    ...
    public String queryPrivate(Method method, String otp, Map<String, String> parameters) throws IOException, NoSuchAlgorithmException, InvalidKeyException {
        ...
        byte[] sha256 = edu.self.kraken.api.KrakenUtils.sha256(nonce + postData);

        // set the API method and retrieve the path
        byte[] path = edu.self.kraken.api.KrakenUtils.stringToBytes(request.setMethod(method));

        // decode the API secret, it's the HMAC key
        byte[] hmacKey = edu.self.kraken.api.KrakenUtils.base64Decode(secret);

        // create the HMAC message from the path and the previous hash
        byte[] hmacMessage = edu.self.kraken.api.KrakenUtils.concatArrays(path, sha256);

        // create the HMAC-SHA512 digest, encode it and set it as the request signature
        String hmacDigest = edu.self.kraken.api.KrakenUtils.base64Encode(edu.self.kraken.api.KrakenUtils.hmacSha512(hmacKey, hmacMessage));
